...
def main():
    ...
    try:
        ...
        for item in result.get("jobs",[]):
            ...
            job={"id":job_id,"title":item["title"],"company":item["company"],"location":item["location"],"category":item["category"],"match":max(0,min(100,int(item["match"]))),"fresh":is_fresh,"status":"active","deadline":item.get("deadline") or None,"url":item["url"],"fit":item["fit"],"gaps":item["gaps"] or None}
        ...
        report={"date":TODAY,"activeJobs":active_count,"freshJobs":fresh_count,"previouslyListed":active_count-fresh_count,"closingWithin7Days":closing,"jobIds":today_ids,"freshJobIds":fresh_ids,"excludedJobIds":[j["id"] for j in existing_jobs if j.get("status")=="excluded" and j.get("id","").endswith(TODAY)]}
        ...
        jobs_data["lastUpdated"]=TODAY; jobs_data["jobs"]=existing_jobs; reports_data["reports"]=existing_reports
